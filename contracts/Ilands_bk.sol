...
contract Iland is ERC721 {
    ...
    function buyNFT(uint256 tokenId, int quot) public payable {
        ...
        for (uint i = 0; i < curatorsAddressSize; i++){
            ...
            */
            ...
            uint rePost = responsebuf[1]; //assin reIntroduce value

            amount = msg.value * (like + rePost) / 100;
            Address.sendValue(payable (curatorsAddress[tokenId][i]) , amount);
        }

        amount = msg.value * 70 / 100;
